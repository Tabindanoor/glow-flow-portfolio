
import { useState, useEffect, useRef } from 'react';
import { Canvas } from '@react-three/fiber';
import { PerspectiveCamera, useTexture, Environment } from '@react-three/drei';
import * as THREE from 'three';
import { motion } from 'framer-motion';
import { staggerFadeIn } from '@/lib/animations';
import { ExternalLink } from 'lucide-react';

interface Project {
  id: number;
  title: string;
  description: string;
  image: string;
  tags: string[];
  link: string;
}

const ProjectCard3D = ({ project, isHovered }: { project: Project; isHovered: boolean }) => {
  const texture = useTexture(project.image || "/placeholder.svg");
  const mesh = useRef<THREE.Mesh>(null!);
  
  useEffect(() => {
    if (!mesh.current) return;
    
    const targetRotationY = isHovered ? Math.PI * 0.05 : 0;
    const targetRotationX = isHovered ? -Math.PI * 0.03 : 0;
    
    // Animate rotation and scale with smoother transitions
    const animateProps = () => {
      mesh.current.rotation.y = THREE.MathUtils.lerp(mesh.current.rotation.y, targetRotationY, 0.05);
      mesh.current.rotation.x = THREE.MathUtils.lerp(mesh.current.rotation.x, targetRotationX, 0.05);
      
      const targetScale = isHovered ? 1.08 : 1;
      const currentScale = mesh.current.scale.x;
      const newScale = THREE.MathUtils.lerp(currentScale, targetScale, 0.1);
      mesh.current.scale.set(newScale, newScale, newScale);
      
      requestAnimationFrame(animateProps);
    };
    
    const animationFrame = requestAnimationFrame(animateProps);
    return () => cancelAnimationFrame(animationFrame);
  }, [isHovered]);
  
  return (
    <mesh ref={mesh} position={[0, 0, 0]} castShadow receiveShadow>
      <boxGeometry args={[3, 2, 0.15]} />
      <meshStandardMaterial
        map={texture}
        metalness={0.7}
        roughness={0.2}
        color={isHovered ? "#9B5DE5" : "#6930C3"}
        emissive={isHovered ? "#9B5DE5" : "#6930C3"}
        emissiveIntensity={isHovered ? 0.4 : 0.2}
      />
    </mesh>
  );
};

const Projects = () => {
  const [hoveredProject, setHoveredProject] = useState<number | null>(null);
  const sectionRef = useRef<HTMLElement>(null);
  const headingRef = useRef<HTMLHeadingElement>(null);
  const projectsRef = useRef<HTMLDivElement>(null);

  const projects: Project[] = [
    {
      id: 1,
      title: "3D Portfolio Website",
      description: "Interactive portfolio with Three.js animations and immersive 3D elements",
      image: "/placeholder.svg",
      tags: ["Three.js", "React", "GSAP"],
      link: "#"
    },
    {
      id: 2,
      title: "E-commerce Platform",
      description: "Fully responsive e-commerce site with 3D product visualization",
      image: "/placeholder.svg",
      tags: ["Next.js", "Three.js", "Stripe"],
      link: "#"
    },
    {
      id: 3,
      title: "3D Data Visualization",
      description: "Interactive dashboard with 3D charts and real-time data visualization",
      image: "/placeholder.svg",
      tags: ["D3.js", "Three.js", "WebGL"],
      link: "#"
    },
    {
      id: 4,
      title: "WebGL Game",
      description: "Browser-based 3D game with physics and interactive elements",
      image: "/placeholder.svg",
      tags: ["WebGL", "Cannon.js", "React"],
      link: "#"
    }
  ];

  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        const [entry] = entries;
        if (entry.isIntersecting) {
          if (headingRef.current) headingRef.current.classList.add('animate-fadeIn');
          
          if (projectsRef.current) {
            const projectElements = Array.from(projectsRef.current.children);
            staggerFadeIn(projectElements, 0.15);
          }
          
          observer.disconnect();
        }
      },
      { threshold: 0.2 }
    );

    if (sectionRef.current) {
      observer.observe(sectionRef.current);
    }

    return () => {
      observer.disconnect();
    };
  }, []);

  return (
    <section 
      id="projects" 
      ref={sectionRef}
      className="py-20 min-h-screen flex items-center bg-gradient-to-b from-dark/95 to-dark relative overflow-hidden"
    >
      {/* Decorative glowing orbs */}
      <div className="absolute top-40 -left-32 w-80 h-80 rounded-full bg-neon-purple/10 blur-[100px] animate-pulse-glow" />
      <div className="absolute bottom-40 -right-32 w-80 h-80 rounded-full bg-neon-cyan/10 blur-[100px] animate-pulse-glow" />
      
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className="text-center"
        >
          <h2 ref={headingRef} className="section-heading text-center">
            My <span className="text-gradient">Projects</span>
          </h2>
          
          <p className="section-subheading text-center opacity-0 animate-fadeIn" style={{ animationDelay: '0.3s' }}>
            Explore my recent work showcasing creative solutions and technical skills
          </p>
        </motion.div>
        
        <div 
          ref={projectsRef} 
          className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-16"
        >
          {projects.map((project) => (
            <motion.div
              key={project.id}
              className="neon-card overflow-hidden opacity-0 group"
              onMouseEnter={() => setHoveredProject(project.id)}
              onMouseLeave={() => setHoveredProject(null)}
              whileHover={{ y: -10 }}
              transition={{ duration: 0.3 }}
            >
              <div className="h-64 relative">
                <Canvas shadows dpr={[1, 2]}>
                  <ambientLight intensity={0.6} />
                  <spotLight 
                    position={[10, 10, 10]} 
                    angle={0.15} 
                    penumbra={1} 
                    intensity={1.5} 
                    castShadow 
                  />
                  <spotLight 
                    position={[-10, -10, -10]} 
                    angle={0.15} 
                    penumbra={1} 
                    intensity={0.5} 
                    color="#00F5FF"
                  />
                  <ProjectCard3D 
                    project={project} 
                    isHovered={hoveredProject === project.id} 
                  />
                  <PerspectiveCamera makeDefault position={[0, 0, 4]} fov={40} />
                  <Environment preset="city" />
                </Canvas>
                
                {/* Gradient overlay for better text readability */}
                <div className="absolute bottom-0 left-0 right-0 h-1/4 bg-gradient-to-t from-dark/80 to-transparent" />
              </div>
              
              <div className="p-8 relative">
                <h3 className="text-2xl font-bold font-space mb-3 text-white group-hover:text-neon-cyan transition-colors">
                  {project.title}
                </h3>
                
                <p className="text-gray-300 mb-6 leading-relaxed">{project.description}</p>
                
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.tags.map((tag) => (
                    <span 
                      key={tag} 
                      className="bg-neon-purple/10 text-neon-cyan text-xs font-medium px-3 py-1.5 rounded-full border border-neon-purple/20"
                    >
                      {tag}
                    </span>
                  ))}
                </div>
                
                <motion.a 
                  href={project.link}
                  className="inline-flex items-center text-neon-cyan hover:text-white font-medium transition-colors group-hover:translate-x-1 duration-300"
                  whileHover={{ x: 4 }}
                  transition={{ type: "spring", stiffness: 400, damping: 10 }}
                >
                  View Project
                  <ExternalLink className="w-4 h-4 ml-2" />
                </motion.a>
                
                {/* Decorative line */}
                <motion.div 
                  className="h-0.5 w-0 bg-gradient-to-r from-neon-purple to-neon-cyan mt-2 group-hover:w-full transition-all duration-700"
                  initial={{ width: 0 }}
                  whileInView={{ width: "30%" }}
                  viewport={{ once: true }}
                />
              </div>
            </motion.div>
          ))}
        </div>
        
        {/* View all projects button */}
        <div className="flex justify-center mt-12">
          <motion.a
            href="#"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.98 }}
            className="btn-primary"
          >
            View All Projects
          </motion.a>
        </div>
      </div>
    </section>
  );
};

export default Projects;
